...
class ProgressTracker:
    ...
    def get_progress(self, goal: Goal, execution_plan: ExecutionPlan) -> Dict[str, Any]:
        ...
        completed_tasks = sum(1 for t in execution_plan.tasks if t.status == TaskStatus.COMPLETED)
        failed_tasks = sum(1 for t in execution_plan.tasks if t.status == TaskStatus.FAILED)
        in_progress_tasks = sum(1 for t in execution_plan.tasks if t.status == TaskStatus.IN_PROGRESS)
        pending_tasks = sum(1 for t in execution_plan.tasks if t.status == TaskStatus.PENDING)
        ...
        if execution_plan.estimated_duration and elapsed_time:
            estimated_remaining = max(0, execution_plan.estimated_duration * 60 - elapsed_time)
    ...
    def get_task_status_summary(self, execution_plan: ExecutionPlan) -> List[Dict[str, Any]]:
        """
